...
for ctx, idx in ctx_list
  ...
  do (ctx)->
    while working
      ...
      await ctx.chunk_pack_s1 pack_key_buf, work_item_i, work_item_o, config.packing_iter, defer()
      await ctx.chunk_pack_s2 pack_key_buf, work_item_i, work_item_o, config.packing_iter, defer()
