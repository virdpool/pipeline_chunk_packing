void _Randomx_context_free_impl(
  std::string*& err,
  Randomx_context* _this
) {
  if (_this->vm_ptr) {
    randomx_destroy_vm(_this->vm_ptr);
    _this->vm_ptr = nullptr;
  }
  
  if (_this->dataset_ptr && !_this->is_dataset_copy) {
    randomx_release_dataset(_this->dataset_ptr);
    _this->dataset_ptr = nullptr;
  }
  
  if (_this->cache_ptr && !_this->is_dataset_copy) {
    randomx_release_cache(_this->cache_ptr);
    _this->cache_ptr = nullptr;
  }
}
...
//   sync
////////////////////////////////////////////////////////////////////////////////////////////////////
napi_value Randomx_context_free_sync(napi_env env, napi_callback_info info) {
  FN_ARG_HEAD_EMPTY
  ...
  _Randomx_context_free_impl(err, _this);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Randomx_context_free {
  Randomx_context* _this;
...
void _worker_ctx_Randomx_context_free_clear(napi_env env, struct Worker_ctx_Randomx_context_free* worker_ctx) {
  if (worker_ctx->err) {
...
void _execute_Randomx_context_free(napi_env env, void* _data) {
  struct Worker_ctx_Randomx_context_free* worker_ctx = (struct Worker_ctx_Randomx_context_free*)_data;
  _Randomx_context_free_impl(worker_ctx->err, worker_ctx->_this);
}

void _complete_Randomx_context_free(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Randomx_context_free* worker_ctx = (struct Worker_ctx_Randomx_context_free*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Randomx_context_free_clear(env, worker_ctx);
    return;
  }
...
napi_value Randomx_context_free(napi_env env, napi_callback_info info) {
  FN_ARG_HEAD(1)
  FN_ARG_THIS(Randomx_context)
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Randomx_context_free* worker_ctx = new Worker_ctx_Randomx_context_free;
  worker_ctx->err = nullptr;
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Randomx_context_free,
    _complete_Randomx_context_free,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
