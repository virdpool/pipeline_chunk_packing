void _Randomx_pipeline_node_task_push_impl(
  std::string*& err,
  Randomx_pipeline_node* _this,
  Randomx_pipeline_task* task,
  i32&         ret
) {
  ...
  Message_ring<Randomx_pipeline_task>* ring = _this->pad_i_list[0];
  if (!ring) {
    err = new std::string("null pad (ring)");
    return;
  }
  ret = ring->push_and_get_free_nonbackpressure_push_size();
  task->task_uid = ++randomx_pipeline_global_task_uid;
  if (task->task_uid < 0) {
    task->task_uid = randomx_pipeline_global_task_uid = 1;
  }
...
//   sync
////////////////////////////////////////////////////////////////////////////////////////////////////
napi_value Randomx_pipeline_node_task_push_sync(napi_env env, napi_callback_info info) {
  FN_ARG_HEAD(1)
  FN_ARG_THIS(Randomx_pipeline_node)
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  FN_ARG_CLASS(Randomx_pipeline_task, task)
  
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  std::string *err = nullptr;
  i32 ret;
  _Randomx_pipeline_node_task_push_impl(err, _this, task, ret);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Randomx_pipeline_node_task_push {
  Randomx_pipeline_node* _this;
  Randomx_pipeline_task* task;
  napi_ref task_ref;
  i32 ret;
...
void _worker_ctx_Randomx_pipeline_node_task_push_clear(napi_env env, struct Worker_ctx_Randomx_pipeline_node_task_push* worker_ctx) {
  if (worker_ctx->err) {
    delete worker_ctx->err;
    worker_ctx->err = nullptr;
  }
  napi_status status;
  if (worker_ctx->task_ref) {
    status = napi_delete_reference(env, worker_ctx->task_ref);
    if (status != napi_ok) {
      printf("status = %d\n", status);
      napi_throw_error(env, nullptr, "napi_delete_reference fail for task");
      return;
    }
  }
...
void _execute_Randomx_pipeline_node_task_push(napi_env env, void* _data) {
  struct Worker_ctx_Randomx_pipeline_node_task_push* worker_ctx = (struct Worker_ctx_Randomx_pipeline_node_task_push*)_data;
  _Randomx_pipeline_node_task_push_impl(worker_ctx->err, worker_ctx->_this, worker_ctx->task, worker_ctx->ret);
}

void _complete_Randomx_pipeline_node_task_push(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Randomx_pipeline_node_task_push* worker_ctx = (struct Worker_ctx_Randomx_pipeline_node_task_push*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Randomx_pipeline_node_task_push_clear(env, worker_ctx);
    return;
  }
  ...
  status = napi_create_int32(env, worker_ctx->ret, &call_argv[1]);
  if (status != napi_ok) {
    fprintf(stderr, "status = %d\n", status);
...
napi_value Randomx_pipeline_node_task_push(napi_env env, napi_callback_info info) {
  FN_ARG_HEAD(2)
  FN_ARG_THIS(Randomx_pipeline_node)
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  FN_ARG_CLASS_VAL(Randomx_pipeline_task, task)
  napi_value callback = argv[arg_idx];
  
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Randomx_pipeline_node_task_push* worker_ctx = new Worker_ctx_Randomx_pipeline_node_task_push;
  worker_ctx->err = nullptr;
  worker_ctx->task_ref = nullptr;
  ...
  worker_ctx->task = task;
  status = napi_create_reference(env, task_val, 1, &worker_ctx->task_ref);
  if (status != napi_ok) {
    printf("status = %d\n", status);
    napi_throw_error(env, nullptr, "Unable to create reference for task. napi_create_reference");
    _worker_ctx_Randomx_pipeline_node_task_push_clear(env, worker_ctx);
    return ret_dummy;
  }
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Randomx_pipeline_node_task_push,
    _complete_Randomx_pipeline_node_task_push,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
